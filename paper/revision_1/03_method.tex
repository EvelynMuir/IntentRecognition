...
We interpret visual intention recognition as a functionally decoupled ambiguity problem: semantic ambiguity primarily distorts local candidate semantics, while supervisory ambiguity primarily destabilizes semantic supervision. \revised{Below we give a concise information-theoretic account of why the two are most naturally addressed by two separately parameterized functions.}

\revised{This view is motivated by a characteristic failure pattern in visual intention recognition: a visual baseline typically retrieves the correct intent into a small local neighborhood, yet fails to rank or calibrate it within that neighborhood. Empirically on Intentonomy, the ground-truth intent almost always lies within the baseline's top few candidates, and the large majority of its false negatives are mis-rankings inside this set rather than genuine retrieval failures, so that an oracle restricted to the same local candidates would already approach the performance ceiling. The dominant difficulty is therefore local ambiguity resolution, ranking, calibrating, and separating the correct intent from adjacent distractors, rather than global retrieval. This motivates modeling semantic ambiguity as feature-side aliasing confined to a local candidate geometry.}
