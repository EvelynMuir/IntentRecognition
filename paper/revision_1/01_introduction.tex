...
Recent studies have increasingly argued that such disagreement should not be treated as mere annotation noise, but as meaningful uncertainty that reflects the structure of subjective interpretation itself \cite{peterson_human_2019, mostafazadeh_davani_dealing_2022}.
...
IntCLIP \cite{leonardis_synergy_2025} incorporates the vision-language alignment knowledge of CLIP into the task, while IntentMLM \cite{wang_unlocking_2026} further leverages multimodal large models, caption generation, and retrieval to perform more holistic intention perception. \revised{This line of work reflects a broader recent shift toward large vision-language and multimodal large language models (MLLMs) as sources of external semantic knowledge for subjective image understanding. However, as our zero-shot experiments in Section~\ref{sec:experiments} show, prompting a general-purpose MLLM directly remains below a task-trained model on this benchmark: such models supply useful intent priors but do not, on their own, provide the dataset-specific calibration and uncertainty-aware supervision that subjective intent recognition requires. Our purpose is therefore not to replace these models, but to convert their semantic knowledge into a structured prior and an uncertainty-gated training signal that explicitly target the two ambiguities below.}
...
\begin{itemize}
    \item We reformulate visual intention recognition as a \emph{functionally decoupled} ambiguity problem, \revised{treating supervisory ambiguity and semantic ambiguity as two separately justified functional roles rather than a single undifferentiated mechanism; this decomposition matches a strong unified alternative in accuracy while keeping each function individually interpretable, separately measurable, and independently deployable.}
