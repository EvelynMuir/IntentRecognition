...
\hline
CLIP ViT-L/14 \cite{radford_learning_2021} & ICML 2021 & $47.07_{\pm0.83}$ & $56.41_{\pm0.53}$ & $56.63_{\pm0.85}$ & $53.37_{\pm0.72}$ \\
FDIL (UTD only) & Ours & $50.20_{\pm0.44}$ & $58.03_{\pm0.11}$ & $58.79_{\pm0.78}$ & $55.68_{\pm0.44}$ \\
FDIL & Ours & $\mathbf{51.14}_{\pm0.39}$ & $\mathbf{59.35}_{\pm0.35}$ & $\mathbf{59.70}_{\pm0.65}$ & $\mathbf{56.73}_{\pm0.27}$ \\
\hline
\end{tabular}
\endgroup
\end{table}
...
\hline
CLIP baseline & Global & $43.84_{\pm1.74}$ & $54.73_{\pm0.58}$ & $54.67_{\pm0.54}$ & $51.08_{\pm0.63}$ & $50.98_{\pm0.49}$ & $22.28_{\pm3.93}$ \\
CLIP baseline & Class-wise & $47.07_{\pm0.83}$ & $56.41_{\pm0.53}$ & $56.63_{\pm0.85}$ & $53.37_{\pm0.72}$ & $50.98_{\pm0.49}$ & $27.19_{\pm1.14}$ \\
\hline
FDIL & Global & $47.89_{\pm1.57}$ & $56.87_{\pm0.67}$ & $56.96_{\pm0.69}$ & $53.91_{\pm0.91}$ & $\mathbf{54.55}_{\pm0.61}$ & $30.73_{\pm3.07}$ \\
FDIL & Class-wise & $\mathbf{51.14}_{\pm0.39}$ & $\mathbf{59.35}_{\pm0.35}$ & $\mathbf{59.70}_{\pm0.65}$ & $\mathbf{56.73}_{\pm0.27}$ & $\mathbf{54.55}_{\pm0.61}$ & $\mathbf{32.12}_{\pm1.26}$ \\
\hline
\end{tabular}
\end{table}

\subsubsection{Method vs. Calibration: A Controlled Decomposition}

To separate the contribution of FDIL's mechanisms from the contribution of class-wise calibration, we evaluate both the CLIP baseline and FDIL under matched global and class-wise thresholding, with all thresholds searched on the validation split and all numbers averaged over 3 seeds (Table~\ref{tab:thresholding_test}). Three observations follow.

First, \emph{FDIL improves over the CLIP baseline under both thresholding protocols}. Under class-wise thresholding the average-F1 gain is $+3.36$ ($53.37\!\rightarrow\!56.73$) with a $+4.93$ gain on the hard subset, and under global thresholding the gain is $+2.83$ in average F1 and $+8.45$ on the hard subset. The improvement is therefore not an artifact of a particular decision rule.

Second, \emph{class-wise thresholding benefits the baseline and FDIL by comparable amounts} ($+2.29$ vs.\ $+2.82$ average F1), so calibration is a shared, method-agnostic post-processing gain rather than something that FDIL uniquely exploits. This directly addresses the concern that the reported improvements might be driven mainly by per-class thresholding: the calibration effect is present for the baseline as well, while FDIL's advantage sits on top of it.

Third, and most decisively, the \emph{threshold-free} mAP confirms the same conclusion: FDIL improves mAP from $50.98_{\pm0.49}$ to $54.55_{\pm0.61}$ ($+3.57$), a metric that is invariant to the thresholding choice by construction. We note that the global-threshold F1 metrics carry large seed variance (e.g., hard-subset std up to $\pm3.9$), because a single shared threshold is a brittle operating point; we therefore treat class-wise F1 and mAP as the primary metrics. For completeness, the average-F1 value of $53.37$ for the CLIP baseline corresponds to class-wise thresholding (Avg.\ = mean of Macro/Micro/Samples F1), not mAP. Overall, per-class calibration is an integral part of the final FDIL decision rule, but it is not the source of FDIL's advantage over the baseline.
