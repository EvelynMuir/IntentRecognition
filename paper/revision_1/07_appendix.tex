...
% FDIL & \textbf{79.66} & \textbf{61.73} & \textbf{49.65} & \textbf{63.68} \\
% \hline
% \end{tabular}
% \end{table}
...
% FDIL & \textbf{63.13} & \textbf{68.33} & \textbf{56.99} & \textbf{62.82} \\
% \hline
% \end{tabular}
% \end{table}
...
% FDIL (full) & 59.34 & \textbf{63.06} & 63.51 & 61.97 & 49.65 \\
% \hline
% \end{tabular}
% \end{table}
...
% ResNet101 & FDIL (full) & 46.15 & 49.29 & 50.98 & 48.81 & 31.67 \\
% \hline
% CLIP ViT-L/14 & Baseline & 55.80 & 59.62 & 60.10 & 58.51 & 40.48 \\
% CLIP ViT-L/14 & FDIL (full) & \textbf{59.34} & \textbf{63.06} & \textbf{63.51} & \textbf{61.97} & \textbf{49.65} \\
% \hline
% \end{tabular}
% \end{table}
...
% Overall, these validation results support the same conclusion as the test split: FDIL improves robustness and discriminability in a consistent manner, rather than relying on split-specific effects.
...
We use an instruction template to generate scenario descriptions for each intent category, which are later encoded as textual priors in SLR-C.
...
\textbf{Target Intent}
The current intent category to describe is: [INTENT]
...
% \subsection{Validation Results for Main Comparison}
% \label{app:validation_results_main}

% This section reports the validation set counterpart of the main comparison in Section~\ref{tab:main_results_val}. We keep the same model groups, metric definitions, and reporting metrics as the test set table in the main text.

% % The validation trend is consistent with the test set findings: the strong CLIP baseline already establishes a high starting point, UTD-only training brings a clear additional gain, and the full FDIL configuration achieves the strongest overall performance.

\section{Validation Results for Subset Evaluation}
\label{app:validation_results_subsets}

We provide validation set counterparts for additional subset evaluation analyses in Tables~\ref{tab:difficulty_results_val} and \ref{tab:content_results_val}.
The result verify that the gains remain stable under both difficulty-based and content-dependent partitions.
...
Method & Easy & Medium & Hard & Avg. \\
\hline
CLIP ViT-L/14 \cite{radford_learning_2021} & \revised{80.10} & \revised{60.30} & \revised{40.03} & \revised{60.14} \\
\revised{FDIL} & \revised{\textbf{80.58}} & \revised{\textbf{62.27}} & \revised{\textbf{50.02}} & \revised{\textbf{64.29}} \\
\hline
\end{tabular}
\end{table}
...
Method & Object & Context & Others & Avg. \\
\hline
CLIP ViT-L/14 \cite{radford_learning_2021} & \revised{62.08} & \revised{64.17} & \revised{51.70} & \revised{59.32} \\
\revised{FDIL} & \revised{\textbf{64.45}} & \revised{\textbf{67.42}} & \revised{\textbf{57.37}} & \revised{\textbf{63.08}} \\
\hline
\end{tabular}
\end{table}
...
% \centering
% \small
% \caption{Validation set results corresponding to Table~\ref{tab:main_results}. We report Macro F1, Micro F1, Samples F1, and their average (Avg.). All numbers are percentages.}
...
% IntentMLM \cite{wang_unlocking_2026} & PR 2026 & 43.06 & 54.77 & 56.93 & 51.59 \\
% \hline
% CLIP ViT-L/14 \cite{radford_learning_2021} & ICML 2021 & 55.80 & 59.62 & 60.10 & 58.51 \\
% FDIL (UTD only) & Ours & 57.86 & 62.16 & 63.88 & 61.30 \\
% FDIL & Ours & \textbf{59.34} & \textbf{63.06} & \textbf{63.51} & \textbf{61.97} \\
% \hline
% \end{tabular}
% \endgroup
% \end{table}