...
This intent is further expanded into multiple scenario archetypes with distinct visual realizations; \revised{for \textit{SocialLifeFriendship}, scenarios include friends hiking together, close friends talking on a couch, a group dinner, or volunteers gardening together.}
...
We interpret visual intention recognition as a functionally decoupled ambiguity problem: semantic ambiguity primarily distorts local candidate semantics, while supervisory ambiguity primarily destabilizes semantic supervision. \revised{A concise information-theoretic account explains why two separately parameterized functions are preferable. Empirically on Intentonomy, a visual baseline almost always retrieves the ground-truth intent into its top few candidates and fails mainly by mis-ranking within this set, so the dominant difficulty is local ambiguity resolution rather than global retrieval, which motivates modeling semantic ambiguity as feature-side aliasing confined to a local candidate geometry.}

\revised{Let $\phi=f_{\text{img}}(X)$ be the frozen image representation, $Y\in\{0,1\}^C$ the latent intent, $\tilde{y}_c$ the noisy annotation, and $\eta_c(x)=\Pr(Y_c{=}1\mid X{=}x)$ the Bayes posterior. For class $c$, the per-sample excess risk decomposes into a label-side and a feature-side term:
\begin{equation}
...
\;+\;\text{est.},
\end{equation}
where $\mathcal{H}$ is the binary entropy and ``est.'' the estimation error. Term~(S) depends only on $\Pr(Y\mid X)$, is large where annotators disagree, and is irreducible by any function of $\phi$; term~(F) depends only on $\phi$ through the inter-class mutual-information overlap
\begin{equation}
\label{eq:aliasing}
\text{aliasing}_c(\phi)\;\propto\;\max_{c'\neq c}\; I\big(\mathbf{1}[Y_c{=}1];\,\phi\big)\;-\;I\big(\mathbf{1}[Y_c{=}1];\,\phi \,\big|\, \mathbf{1}[Y_{c'}{=}1]\big),
\end{equation}
i.e.\ the class-$c$ evidence in $\phi$ that is inseparable from a neighbor $c'$, and is reduced only by restructuring the feature-side geometry. Because the two terms are functionals of different objects, a single mixed-target mechanism must trade one ambiguity against the other, whereas a decoupled realization need not.}

\revised{The two FDIL modules instantiate exactly these corrections: SLR-C attacks term~(F) by contracting the candidate set with the semantic prior and adding image-specific residual directions $h_{\text{res}}(\phi)$, lowering $\text{aliasing}_c(\phi)$ without touching the supervision target, while UTD attacks term~(S) by down-weighting the direct loss through the agreement gate $g=1-\omega$ where $\mathcal{H}(\eta_c(X))$ is high and substituting a smoother rationale-teacher distribution.} This functionally decoupled design enables effective handling of ambiguity without introducing heavy architectural complexity.
