...
We interpret visual intention recognition as a functionally decoupled ambiguity problem: semantic ambiguity primarily distorts local candidate semantics, while supervisory ambiguity primarily destabilizes semantic supervision. \revised{A short risk-decomposition argument explains why two separately parameterized functions are preferable. Let $\phi=f_{\text{img}}(X)$ be the frozen image feature, $\tilde{y}_c$ the noisy annotation, and $\eta_c(x)=\Pr(Y_c{=}1\mid X{=}x)$ the Bayes posterior. For class $c$, the per-sample excess risk splits into a label-side and a feature-side term:
\begin{equation}
