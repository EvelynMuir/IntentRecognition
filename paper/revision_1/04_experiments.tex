...
\centering
\small
\caption{Comparison with state-of-the-art methods. We report Macro F1, Micro F1, Samples F1, and Avg. (the mean of the three F1 scores).}
...
\begin{table}[tb]
\centering
\caption{Performance on difficulty-based intent subsets (Easy/Medium/Hard). Avg. denotes the mean over the three subsets.}
...
\begin{table}[tb]
\centering
\caption{Performance on content-dependent intent subsets (Object, Context, and Others). Avg. denotes the mean over the three subsets.}
...
\begin{table}[tb]
\centering
\caption{Component ablation of FDIL. \textit{w/o UTD} removes uncertainty-guided text distillation, and \textit{w/o Gated} removes agreement-aware gating.}
...
\centering
\small
\caption{\revised{Unified versus functionally decoupled ambiguity handling. Unified joint-target KD uses a single mixed target from SLR-C and the rationale teacher, without residual decomposition or agreement-aware gating.}}
...
We next compare different textual priors used by SLR-C. We consider three semantic sources: \textit{Lexical}, which uses short class-name phrases; \textit{Canonical}, which uses the official intent descriptions provided by Intentonomy; and \textit{Scenario}, which uses Gemini-generated visual scenarios. For the ensemble setting, we average the normalized logits from the selected sources before local reranking. This table therefore isolates the effect of prior sources under the SLR-C reranking stage only.
\begin{table}[tb]
\centering
\caption{\revised{Ablation of lexical, canonical, and scenario textual priors in SLR-C (reranking-only, $K{=}10$). Ensemble averages normalized logits from selected priors.}}
...
\begin{table}[tb]
\centering
\caption{Sensitivity to candidate size $K$. Thresholding is fixed to class-wise search on the validation split. Avg. denotes the mean of Macro, Micro, and Samples F1.}
...
\centering
\small
\caption{\revised{Method-vs-calibration decomposition ($K{=}10$): CLIP baseline and FDIL under matched global and class-wise thresholding (validation-searched). FDIL is mean$\pm$std over 3 seeds; mAP is threshold-free.}}
...
\revised{To separate the contribution of FDIL's mechanisms from that of class-wise calibration, we evaluate both the CLIP baseline and FDIL under matched global and class-wise thresholding, as shown in Table~\ref{tab:thresholding_test}.}
\revised{First, FDIL improves over the CLIP baseline under both thresholding protocols, on both average and hard-subset F1, so the improvement is not an artifact of a particular decision rule.}
\revised{Second, class-wise thresholding benefits both the baseline and FDIL, so calibration is a shared post-processing gain rather than something unique to FDIL. This addresses the concern that the reported improvements might be driven mainly by per-class thresholding.}
\revised{Third, and most decisively, the threshold-free mAP confirms the same conclusion that FDIL improves mAP by $+4.4$ points, a metric invariant to the thresholding choice by construction. Overall, per-class calibration is an integral part of the final decision rule, but it is not the source of FDIL's advantage over the baseline.}

\subsubsection{\revised{Threshold-free Evaluation and Statistical Significance}}

\revised{To remove any residual dependence on the decision threshold and to test whether the gains are statistically reliable, we additionally report fully threshold-free, rank-based metrics, mean average precision (mAP, the macro PR-AUC), macro and micro ROC-AUC, and micro PR-AUC. Table~\ref{tab:threshold_free} reports the metrics for FDIL and its ablations, while Figure~\ref{fig:roc_pr} additionally compares FDIL against representative previous methods reproduced under the matched CLIP ViT-L/14 protocol, using the corresponding micro-averaged ROC and precision-recall curves.}

\begin{table}[tb]
\centering
\small
\caption{\revised{Threshold-free, rank-based metrics; mAP is the macro average precision (production values, as in Table~\ref{tab:thresholding_test}). ROC/PR-AUC use a representative seed; paired-bootstrap deltas are reported in the text.}}
...
\begin{table}[tb]
\centering
\caption{Ablation of rationale construction for UTD. Step 1/2/3 correspond to visual evidence, contextual bridging, and counterfactual disambiguation.}
...
\centering
\small
\caption{\revised{Ablations of the UTD text teacher (UTD without SLR-C). Top block: effect of agreement-aware gating. Bottom block: teacher input modality, reporting teacher-only oracle and the corresponding gated-distillation student. Bold marks the best entry within each comparison.}}
...
Both distillation variants improve substantially over plain supervised learning, and agreement-aware gating provides the best overall balance in Macro and Samples F1, supporting our interpretation that gating acts as a form of \emph{on-demand distillation}: it suppresses over-regularization on reliable samples while reserving teacher guidance for genuinely ambiguous cases. On the modality side, the multimodal oracle teacher is not weaker by itself, but it becomes a worse distillation target, indicating that explicit image-text fusion inside the teacher may introduce noisy or redundant visual evidence, whereas a text-only semantic manifold provides cleaner dark knowledge for the student. The complementary evidence on weaker backbones is reported later in the robustness analysis.
...
\begin{table}[tb]
\centering
\caption{Backbone sensitivity of FDIL.}
...
\begin{table}[tb]
\centering
\caption{\revised{Efficiency on top of frozen CLIP ViT-L/14 features: params (inference-active heads), per-image FLOPs ($\approx2\times$ MACs), and batch-1 latency on an RTX 4090. Previous methods use the same shared-feature protocol; IntentMLM is MLLM-dominated and reported qualitatively.}}
