...
\caption{\revised{Overview of FDIL. SLR-C constructs a textual semantic prior and performs local reranking plus residual refinement. UTD uses a rationale-based text teacher and an agreement-aware gate to stabilize learning under ambiguous supervision.}}
...
\revised{A candidate-recall diagnostic supports this local view. The baseline Top-10 candidates already cover $83.9\%$ of positive labels, and $67.8\%$ of its false negatives remain recoverable within the same set on Intentonomy. Thus, the main bottleneck is often ranking and calibration among retrieved neighboring intents rather than failure to retrieve the correct intent.}

\revised{A short risk-decomposition argument explains why two separately parameterized functions are preferable. Let $\phi=f_{\text{img}}(X)$ be the frozen image feature, $\tilde{y}_c$ the noisy annotation, and $\eta_c(x)=\Pr(Y_c{=}1\mid X{=}x)$ the Bayes posterior. For class $c$, the per-sample excess risk splits into a label-side and a feature-side term:
\begin{equation}
\label{eq:risk_decomp}
\mathbb{E}\!\left[\ell\big(g_c(\phi),\, \tilde{y}_c\big)\right]
\;\approx\;
...
\;+\;\text{est.},
\end{equation}
where $\mathcal{H}$ is the binary entropy. Term~(S) depends only on $\Pr(Y\mid X)$, is large where annotators disagree, and is irreducible by any function of $\phi$; term~(F) measures the class-$c$ evidence in $\phi$ that is inseparable from a neighbor $c'$ and is reduced only by restructuring the feature-side geometry. Because the two terms are functionals of different objects, a single mixed-target mechanism must trade one against the other, whereas a decoupled realization need not. The two FDIL modules instantiate exactly these corrections: SLR-C attacks term~(F) by contracting the candidate set and adding image-specific residual directions $h_{\text{res}}(\phi)$ without touching the supervision target, while UTD attacks term~(S) by down-weighting the direct loss through the agreement gate $g=1-\omega$ where $\mathcal{H}(\eta_c(X))$ is high.} This design handles ambiguity without heavy architectural complexity.
