...
\revised{The scenarios are not hand-picked illustrations but the output of a fixed generation protocol applied uniformly to every intent class (the exact instruction template is released with our artifacts). For each intent we ask the vision-language model to enumerate visually distinct realizations along five complementary axes (subjects/objects, actions/interactions, settings/context, emotion/atmosphere, and photographic style), and we require at least three different settings per intent so that the scenarios span the typical intra-class appearance variation rather than a single canonical view. The criterion for inclusion is thus representativeness of the common visual modes of an intent rather than exhaustive coverage; under situations outside the generated scenarios the prior degrades gracefully toward the lexical and canonical sources, the base visual logits, and the residual student.}
...
We interpret visual intention recognition as a functionally decoupled ambiguity problem: semantic ambiguity primarily distorts local candidate semantics, while supervisory ambiguity primarily destabilizes semantic supervision. \revised{Below we give a concise information-theoretic account of why the two are most naturally addressed by two separately parameterized functions; the full derivation is provided in the response letter. The argument is a modeling principle rather than a claim of unique optimality, which we verify empirically in Section~\ref{sec:experiments}.}

\revised{The principle is motivated by a concrete failure pattern in Intentonomy: a visual baseline usually retrieves the correct intent into a small local neighborhood but fails to rank or calibrate it. The baseline Top-10 set already covers $83.9\%$ of positive labels, $95.1\%$ of test images contain at least one ground-truth intent in that set, and $67.8\%$ of baseline false negatives remain inside it; an oracle selecting from the same Top-10 candidates would reach $87.4/91.3/89.7$ Macro/Micro/Samples F1, far above the realized baseline. The difficulty is therefore largely local ambiguity resolution -- ranking, calibrating, and separating the correct intent from adjacent distractors -- rather than global retrieval failure, which is why we model semantic ambiguity as feature-side aliasing within a local candidate geometry.} % 这一段需要改一下表述风格，现在太像实验报告了，而不是一篇学术论文中的method部分。

\revised{Let $\phi=f_{\text{img}}(X)$ be the frozen image representation, $Y\in\{0,1\}^C$ the latent intent, and $\omega\in(0,1]$ the agreement score of Section~\ref{sec:method}. For class $c$, the per-sample excess risk decomposes into a label-side and a feature-side term:
\begin{equation}
\label{eq:risk_decomp}
\underbrace{\mathbb{E}\!\left[\ell\big(g_c(\phi),\, \tilde{y}_c\big)\right]}_{\text{training risk}}
\;\approx\;
\underbrace{\mathbb{E}_X\!\big[\mathcal{H}(\eta_c(X))\big]}_{\text{(S) supervisory}}
\;+\;
\underbrace{\mathbb{E}_X\!\big[\,\eta_c(X)\,\text{aliasing}_c(\phi)\,\big]}_{\text{(F) semantic}}
\;+\;\text{est.},
\end{equation}
where $\tilde{y}_c$ is the noisy annotation, $\eta_c(x)=\Pr(Y_c{=}1\mid X{=}x)$ the Bayes posterior, $\mathcal{H}$ the binary entropy, and ``est.'' the estimation error. Term~(S) depends only on $\Pr(Y\mid X)$: it is large where annotators disagree and is irreducible by any function of $\phi$, since fitting harder against $\tilde{y}_c$ only inflates target noise. Term~(F) depends only on $\phi$ through the inter-class mutual-information overlap
\begin{equation}
\label{eq:aliasing}
\text{aliasing}_c(\phi)\;\propto\;\max_{c'\neq c}\; I\big(\mathbf{1}[Y_c{=}1];\,\phi\big)\;-\;I\big(\mathbf{1}[Y_c{=}1];\,\phi \,\big|\, \mathbf{1}[Y_{c'}{=}1]\big),
\end{equation}
i.e.\ the part of the class-$c$ evidence in $\phi$ that is not separable from a neighbor $c'$ once that neighbor is known, and is reduced only by restructuring the feature-side geometry. Because the two terms are functionals of different objects, corrections acting on $\omega$ alone and on $(\phi,\text{class text})$ alone have non-interfering first-order effects, so a single mixed-target mechanism must trade one ambiguity against the other whereas a decoupled realization need not.}

\revised{The two FDIL modules instantiate exactly these corrections. SLR-C attacks term~(F): Top-$K$ reranking with the semantic prior contracts the candidate set to the locally confusable neighborhood and the residual $h_{\text{res}}(\phi)$ adds image-specific discriminative directions, lowering $\text{aliasing}_c(\phi)$ without touching the supervision target. UTD attacks term~(S): the agreement gate $g=1-\omega$ down-weights the direct loss where $\mathcal{H}(\eta_c(X))$ is high and substitutes a smoother rationale-teacher distribution, without altering the feature-side geometry.}

FDIL thus addresses the two ambiguities through two different but compatible functions: SLR-C first provides structured candidate semantics through a semantic prior and residual refinement, while UTD supplies agreement-guided semantic supervision that stabilizes both the visual student and, in the full model, the residual refinement built on top of the prior.
