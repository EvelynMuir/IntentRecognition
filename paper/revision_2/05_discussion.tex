...
Despite the consistent gains, the framework has several limitations. First, it depends on offline text generation and external prior construction, increasing preparation cost, even though UTD adds no inference-time cost. Second, the gains shrink on weaker backbones such as ResNet101, indicating that residual refinement still relies on reasonably strong base representations. Third, richer priors are not automatically better unless their interaction with the student is carefully controlled. Fourth, per-class attribution reveals a failure mode: for a few broadly defined, overlapping categories, the residual student can resolve the local candidate set toward a competing neighbor and lose accuracy on the original class.
